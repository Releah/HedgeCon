...
Tagged releases are built by `.github/workflows/release.yml`. The Git tag must match the version in `package.json`—for example, package version `0.1.3` uses tag `v0.1.3`. The workflow publishes the installer, portable executable, blockmap and updater metadata to GitHub Releases. No GitHub token is stored in the application.
