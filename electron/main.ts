...
ipcMain.handle('repo:push', async () => { const repository = getRepository(); if (!repository.remoteUrl) throw new Error('Configure a remote repository before pushing.'); await git.push({ fs, http: gitHttp, dir: repository.localPath, remote: 'origin', ref: repository.branch, onAuth: gitAuth(repository) }); return gitStatus(); });
...
ipcMain.handle('wiki:create', (_event, section: string, name: string) => { if (!['general', 'vendors'].includes(section) || typeof name !== 'string' || !name.trim() || name.length > 100) throw new Error('Enter a valid page name.'); const slug = name.trim().toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, ''); if (!slug) throw new Error('Enter a page name containing letters or numbers.'); const relativePath = section === 'general' ? `wiki/general/${slug}.md` : `vendors/${slug}.md`; const target = safeRepoPath(relativePath); if (fs.existsSync(target)) throw new Error('A page with that name already exists.'); fs.mkdirSync(path.dirname(target), { recursive: true }); fs.writeFileSync(target, `# ${name.trim()}\n\n`, { encoding: 'utf8', flag: 'wx' }); return { path: relativePath, title: name.trim(), section }; });
...
ipcMain.handle('keys:list', () => {
  ...
  for (const location of locations) { if (!fs.existsSync(location.directory)) continue; for (const entry of fs.readdirSync(location.directory, { withFileTypes: true }).slice(0, 200)) { const fullPath = path.join(location.directory, entry.name); if (!entry.isFile() || entry.name.endsWith('.pub') || !/^id_|\.key$|\.pem$|\.pk8$/i.test(entry.name) || seen.has(path.resolve(fullPath).toLowerCase())) continue; try { const stats = fs.statSync(fullPath); if (stats.size > 1024 * 1024) continue; keys.push(keyInfo(fullPath, location.source)); seen.add(path.resolve(fullPath).toLowerCase()); } catch { /* Ignore unreadable key candidates. */ } } }
  return keys;
});
