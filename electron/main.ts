...
ipcMain.handle('ssh:identify', async (_event, connectionId: string, observedText?: string) => {
  ...
  if (!completeNetworkIdentity(execDetected)) { const shellOutput = await sshShellProbe(connection.client, ['terminal length 0', 'terminal width 511', 'show version', 'show inventory', 'show platform', 'show hostname'], 9000).catch(() => ''); networkOutput += `\n${shellOutput}`; const shellDetected = networkIdentity(`${observedText || ''}\n${networkOutput}`); if (shellDetected) return { ...shellDetected, confidence: shellDetected.version || !/network device/i.test(shellDetected.product) ? 'high' : 'medium' }; }
