...
ipcMain.handle('ssh:identify', async (_event, connectionId: string, observedText?: string) => {
  ...
  if (!completeNetworkIdentity(execDetected)) { let shellOutput = ''; try { shellOutput = await sshShellProbe(connection.client, ['terminal length 0', 'terminal width 511', 'show version', 'show inventory', 'show platform', 'show hostname'], 9000); } catch { if (connection.stream) for (const command of ['show version', 'show inventory']) { const output = await sshInteractiveCapture(connectionId, connection.stream, command, 14000, 65536, false).catch(() => ''); shellOutput += `\n${output}`; const detected = networkIdentity(`${observedText || ''}\n${networkOutput}\n${shellOutput}`); if (completeNetworkIdentity(detected)) return detected!; } } networkOutput += `\n${shellOutput}`; const shellDetected = networkIdentity(`${observedText || ''}\n${networkOutput}`); if (shellDetected) return { ...shellDetected, confidence: shellDetected.version || !/network device/i.test(shellDetected.product) ? 'high' : 'medium' }; }
