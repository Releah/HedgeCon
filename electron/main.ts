...
const defaults: StoredData = { folders: [], sessions: [], macros: [], macroFolders: [], serialProfiles: [], credentialProfileMappings: {}, knownHosts: {}, legacySshHosts: {}, wikiGuideSeeded: false, credentialSets: [], inventorySettings: { configured: false, mode: 'local' }, updateSettings: { automaticChecks: true, branch: 'main' }, sessionLogSettings: defaultSessionLogSettings };
...
const starterWiki: Record<string, string> = {
  ...
  'wiki/general/markdown-guide.md': '# Markdown quick guide\n\nA compact reference for writing HedgeCon Wiki pages. Open **Edit** to see the Markdown behind each example.\n\n## Headings\n\n```markdown\n# Page title\n## Main section\n### Smaller section\n```\n\n## Emphasis and lists\n\n```markdown\n**Bold text**\n*Italic text*\n~~Crossed-out text~~\n\n- Bullet item\n- Another item\n  - Nested item\n\n1. First step\n2. Second step\n```\n\n## Links and images\n\n```markdown\n[Link text](https://example.com)\n![Image description](assets/example.png)\n```\n\nYou can paste a Snipping Tool image directly while editing a Wiki page; HedgeCon stores the image beside that page and inserts the image Markdown automatically.\n\n## Tables\n\n```markdown\n| Interface | Address | Status |\n| --- | --- | --- |\n| Gi1/0/1 | 10.0.0.1 | Up |\n| Gi1/0/2 | 10.0.0.2 | Down |\n```\n\nAlignment can be controlled with colons:\n\n```markdown\n| Left | Centre | Right |\n| :--- | :---: | ---: |\n| A | B | C |\n```\n\n## Commands and code\n\nUse single backticks for an inline command such as `show version`. Use three backticks for a block:\n\n````markdown\n```text\nshow interfaces status\nshow logging\n```\n````\n\n## Quotes, separators and checklists\n\n```markdown\n> Important operational note\n\n---\n\n- [x] Completed check\n- [ ] Outstanding check\n```\n\n## Useful habits\n\n- Leave a blank line before and after headings, lists, tables and code blocks.\n- Use descriptive link text rather than pasting long URLs into sentences.\n- Add a language after the opening backticks, such as `bash`, `powershell`, `yaml` or `text`, for clearer code blocks.\n- Preview the page before saving, then commit and sync shared changes when ready.\n',
...
function safeRepoPath(relativePath: string) { if (relativePath === privateWikiPrefix || relativePath.startsWith(`${privateWikiPrefix}/`)) return safePrivateNotesPath(relativePath); const repository = getRepository(); if (typeof relativePath !== 'string' || !relativePath || relativePath.length > 4096 || path.isAbsolute(relativePath) || relativePath.includes('\0')) throw new Error('Invalid repository path.'); const root = path.resolve(repository.localPath); const target = path.resolve(root, relativePath); const compare = (value: string) => process.platform === 'win32' ? value.toLowerCase() : value; if (!compare(target).startsWith(`${compare(root)}${path.sep}`)) throw new Error('Repository path escapes the workspace.'); let cursor = root; for (const segment of path.relative(root, target).split(path.sep)) { cursor = path.join(cursor, segment); if (fs.existsSync(cursor) && fs.lstatSync(cursor).isSymbolicLink()) throw new Error('Symbolic links are not allowed in managed Wiki paths.'); } return target; }
function privateNotesRoot() { return path.join(app.getPath('userData'), 'private-notes'); }
const privateWikiPrefix = 'wiki/general/private-notes';
function safePrivateNotesPath(relativePath: string) { if (typeof relativePath !== 'string' || relativePath.length > 4096 || relativePath.includes('\0')) throw new Error('Invalid private note path.'); const localRelative = relativePath.startsWith(`${privateWikiPrefix}/`) || relativePath === privateWikiPrefix ? `general${relativePath.slice(privateWikiPrefix.length)}` : relativePath.startsWith('private/') ? relativePath.slice('private/'.length) : ''; if (!localRelative) throw new Error('Invalid private note path.'); const root = path.resolve(privateNotesRoot()); const target = path.resolve(root, localRelative); const compare = (value: string) => process.platform === 'win32' ? value.toLowerCase() : value; if (!compare(target).startsWith(`${compare(root)}${path.sep}`)) throw new Error('Private note path escapes local storage.'); return target; }
function wikiStoragePath(relativePath: string) { return relativePath.startsWith('private/') || relativePath === privateWikiPrefix || relativePath.startsWith(`${privateWikiPrefix}/`) ? safePrivateNotesPath(relativePath) : safeRepoPath(relativePath); }
function listPrivateWikiPages() { const pages: Array<{ path: string; title: string; section: 'private' }> = []; const root = path.join(privateNotesRoot(), 'general'); if (!fs.existsSync(root)) return pages; const pending = [root]; while (pending.length) { const directory = pending.pop()!; for (const entry of fs.readdirSync(directory, { withFileTypes: true }).slice(0, 2000)) { if (entry.isSymbolicLink()) continue; const full = path.join(directory, entry.name); if (entry.isDirectory()) pending.push(full); else if (entry.isFile() && entry.name.toLowerCase().endsWith('.md') && fs.statSync(full).size <= 2 * 1024 * 1024) { const suffix = path.relative(root, full).split(path.sep).join('/'); pages.push({ path: `${privateWikiPrefix}/${suffix}`, title: wikiPageTitle(fs.readFileSync(full, 'utf8'), entry.name), section: 'private' }); } } } return pages.sort((a, b) => a.title.localeCompare(b.title)); }
function listPrivateWikiFolders() { const folders: Array<{ path: string; name: string; section: 'private' }> = [{ path: privateWikiPrefix, name: 'Private Notes · Local only', section: 'private' }]; const root = path.join(privateNotesRoot(), 'general'); if (!fs.existsSync(root)) return folders; const pending = [root]; while (pending.length) { const directory = pending.pop()!; for (const entry of fs.readdirSync(directory, { withFileTypes: true }).slice(0, 2000)) { if (!entry.isDirectory() || entry.isSymbolicLink()) continue; const full = path.join(directory, entry.name); folders.push({ path: `${privateWikiPrefix}/${path.relative(root, full).split(path.sep).join('/')}`, name: entry.name.replace(/[-_]/g, ' '), section: 'private' }); pending.push(full); } } return folders.sort((a, b) => a.path.localeCompare(b.path)); }
...
const serialConnections = new Map<string, SerialPort>();
const interactiveCompareCaptures = new Set<string>();
...
let selectedUpdateRelease: UpdateRelease | null = null;
let availableDowngrades = new Map<string, UpdateRelease>();
...
function versionParts(version: string) { const match = version.replace(/^v/i, '').match(/^(\d+)\.(\d+)\.(\d+)(?:-([0-9A-Za-z.-]+))?/); return match ? { numbers: match.slice(1, 4).map(Number), prerelease: match[4] || '' } : null; }
function compareVersions(left: string, right: string) { const a = versionParts(left); const b = versionParts(right); if (!a || !b) return 0; for (let index = 0; index < 3; index += 1) if (a.numbers[index] !== b.numbers[index]) return a.numbers[index] - b.numbers[index]; if (a.prerelease === b.prerelease) return 0; if (!a.prerelease) return 1; if (!b.prerelease) return -1; return a.prerelease.localeCompare(b.prerelease, undefined, { numeric: true }); }
function restoreDefaultUpdateFeed() { selectedUpdateRelease = null; autoUpdater.allowDowngrade = false; autoUpdater.allowPrerelease = stored.updateSettings.branch === 'experimental'; autoUpdater.setFeedURL({ provider: 'github', owner: 'Releah', repo: 'HedgeCon' }); }
async function listDowngradeReleases() {
  if (!app.isPackaged || portableBuild) return [];
  const response = await fetch('https://api.github.com/repos/Releah/HedgeCon/releases?per_page=50', { headers: { Accept: 'application/vnd.github+json', 'User-Agent': `HedgeCon/${app.getVersion()}` } });
  if (!response.ok) throw new Error(`GitHub release lookup failed (${response.status}). Try again later.`);
  const releases = await response.json() as Array<{ tag_name?: unknown; name?: unknown; published_at?: unknown; prerelease?: unknown; draft?: unknown; assets?: Array<{ name?: unknown }> }>;
  const metadataName = process.platform === 'linux' ? 'latest-linux.yml' : process.platform === 'darwin' ? 'latest-mac.yml' : 'latest.yml';
  const filtered = releases.flatMap(release => { const tag = typeof release.tag_name === 'string' ? release.tag_name : ''; const version = tag.replace(/^v/i, ''); const prerelease = release.prerelease === true; const matchesBranch = stored.updateSettings.branch === 'experimental' ? prerelease : !prerelease; const hasMetadata = release.assets?.some(asset => asset.name === metadataName); if (release.draft || !tag || !matchesBranch || !hasMetadata || compareVersions(version, app.getVersion()) >= 0) return []; return [{ tag, version, name: typeof release.name === 'string' && release.name.trim() ? release.name.slice(0, 200) : tag, publishedAt: typeof release.published_at === 'string' ? release.published_at : '', prerelease }]; });
  availableDowngrades = new Map(filtered.map(release => [release.tag, release]));
...
function sessionLogDay(value = new Date()) { const year = value.getFullYear(); const month = String(value.getMonth() + 1).padStart(2, '0'); const day = String(value.getDate()).padStart(2, '0'); return `${year}-${month}-${day}`; }
function logFiles() { const root = sessionLogDirectory(); if (!fs.existsSync(root)) return []; const files: Array<{ path: string; size: number; modified: number }> = []; const pending = [root]; while (pending.length) { const directory = pending.pop()!; for (const entry of fs.readdirSync(directory, { withFileTypes: true })) { if (entry.isSymbolicLink()) continue; const entryPath = path.join(directory, entry.name); if (entry.isDirectory()) pending.push(entryPath); else if (entry.isFile() && entry.name.endsWith('.log')) try { const stats = fs.statSync(entryPath); files.push({ path: entryPath, size: stats.size, modified: stats.mtimeMs }); } catch { /* A file may disappear during pruning. */ } } } return files; }
function pruneEmptyLogDirectories() { const root = sessionLogDirectory(); if (!fs.existsSync(root)) return; const directories: string[] = []; const pending = [root]; while (pending.length) { const directory = pending.pop()!; for (const entry of fs.readdirSync(directory, { withFileTypes: true })) if (entry.isDirectory() && !entry.isSymbolicLink()) { const child = path.join(directory, entry.name); directories.push(child); pending.push(child); } } for (const directory of directories.sort((left, right) => right.length - left.length)) try { if (!fs.readdirSync(directory).length) fs.rmdirSync(directory); } catch { /* Retry after the next prune. */ } }
...
function openSessionLog(connectionId: string, sessionName: string, host: string) {
  if (!stored.sessionLogSettings.enabled) return; try { fs.mkdirSync(sessionLogDirectory(), { recursive: true }); pruneSessionLogs();
    const started = new Date(); const directory = path.join(sessionLogDirectory(), `${safeLogName(sessionName)}_${safeLogName(host)}`, sessionLogDay(started)); fs.mkdirSync(directory, { recursive: true });
    const stamp = started.toISOString().replace(/[:.]/g, '-'); const base = `${stamp}_${connectionId.slice(0, 8)}`;
    const filePath = path.join(directory, `${base}.log`); const header = `HedgeCon SSH session log\nSession: ${sessionName}\nHost: ${host}\nStarted: ${started.toISOString()}\n${'-'.repeat(72)}\n`;
...
function configureUpdates() {
  ...
  autoUpdater.autoDownload = false; autoUpdater.autoInstallOnAppQuit = false; restoreDefaultUpdateFeed();
  autoUpdater.on('checking-for-update', () => publishUpdateStatus({ status: 'checking', message: undefined }));
  autoUpdater.on('update-available', info => publishUpdateStatus({ status: 'available', availableVersion: info.version, releaseNotes: releaseNotesText(info), progress: undefined, message: undefined, downgrade: Boolean(selectedUpdateRelease) }));
  autoUpdater.on('update-not-available', info => publishUpdateStatus({ status: 'current', availableVersion: info.version, progress: undefined, message: selectedUpdateRelease ? `Version ${selectedUpdateRelease.version} is not compatible with this installation.` : 'HedgeCon is up to date.', downgrade: Boolean(selectedUpdateRelease) }));
  ...
  autoUpdater.on('update-downloaded', info => publishUpdateStatus({ status: 'downloaded', availableVersion: info.version, releaseNotes: releaseNotesText(info), progress: 100, message: selectedUpdateRelease ? 'The downgrade is ready to install.' : 'The update is ready to install.', downgrade: Boolean(selectedUpdateRelease) }));
...
function stopSshTunnel(id: string) { const tunnel = sshTunnels.get(id); if (!tunnel) return false; sshTunnels.delete(id); for (const socket of tunnel.sockets) socket.destroy(); tunnel.server.close(); return true; }
function stopConnectionTunnels(connectionId: string) { for (const tunnel of [...sshTunnels.values()]) if (tunnel.connectionId === connectionId) stopSshTunnel(tunnel.id); }
function tunnelMeta(tunnel: (typeof sshTunnels extends Map<string, infer T> ? T : never)) { return { id: tunnel.id, connectionId: tunnel.connectionId, type: tunnel.type, localPort: tunnel.localPort, targetHost: tunnel.targetHost, targetPort: tunnel.targetPort, connections: tunnel.sockets.size }; }
function sshExec(client: Client, command: string, timeoutMs = 5000, maximum = 32768) { return new Promise<string>((resolve, reject) => client.exec(command, (error, stream) => { if (error) return reject(error); let output = ''; let settled = false; const timer = setTimeout(() => { if (!settled) { settled = true; stream.close(); resolve(output); } }, timeoutMs); const collect = (data: Buffer) => { if (output.length < maximum) output += data.toString().slice(0, maximum - output.length); }; stream.on('data', collect); stream.stderr.on('data', collect); stream.on('close', () => { if (!settled) { settled = true; clearTimeout(timer); resolve(output); } }); stream.on('error', (failure: Error) => { if (!settled) { settled = true; clearTimeout(timer); reject(failure); } }); })); }
function sshShellProbe(client: Client, commands: string[], timeoutMs = 9000, maximum = 65536) { return new Promise<string>((resolve, reject) => client.shell({ term: 'dumb', cols: 200, rows: 1000 }, (error, stream) => { if (error) return reject(error); let output = ''; let settled = false; const finish = () => { if (settled) return; settled = true; clearTimeout(timer); try { stream.close(); } catch { /* The probe shell may already be closed. */ } resolve(output); }; const timer = setTimeout(finish, timeoutMs); const collect = (data: Buffer) => { if (output.length < maximum) output += data.toString().slice(0, maximum - output.length); }; stream.on('data', collect); stream.stderr.on('data', collect); stream.on('close', finish); stream.on('error', (failure: Error) => { if (!settled) { settled = true; clearTimeout(timer); reject(failure); } }); stream.write(`${commands.join('\n')}\nexit\n`); })); }
function telemetryPercent(value: number | null) { return value === null || !Number.isFinite(value) ? null : Math.max(0, Math.min(100, value)); }
function parseTelemetry(output: string, source: string) {
  const number = (...patterns: RegExp[]) => { for (const pattern of patterns) { const match = output.match(pattern); if (match) return Number(match[1]); } return null; };
  let cpu = number(/HEDGE_CPU\|([\d.]+)/i, /CPU utilization[^\r\n]*?(?:five seconds|5 seconds)?\s*[:=]?\s*(\d+(?:\.\d+)?)\s*%/i, /CPU utilization\s+(\d+(?:\.\d+)?)\s+percent/i, /CPU[^\r\n]*?idle[^\d]*(\d+(?:\.\d+)?)\s*%/i);
  if (/CPU[^\r\n]*?idle/i.test(output) && cpu !== null) cpu = 100 - cpu;
  let memory = number(/HEDGE_MEM\|([\d.]+)/i, /Memory utilization\s+(\d+(?:\.\d+)?)\s+percent/i, /Memory[^\r\n]*?(\d+(?:\.\d+)?)\s*%/i);
  const pool = output.match(/Processor Pool Total:\s*(\d+)\s+Used:\s*(\d+)/i); if (memory === null && pool) memory = Number(pool[2]) / Number(pool[1]) * 100;
  let storage = number(/HEDGE_DISK\|([\d.]+)/i, /(?:Filesystem|Storage|Disk)[^\r\n]*?(\d+(?:\.\d+)?)\s*%/i);
  const flash = output.match(/(\d+)\s+bytes total\s+\((\d+)\s+bytes free\)/i); if (storage === null && flash) storage = (1 - Number(flash[2]) / Number(flash[1])) * 100;
  cpu = telemetryPercent(cpu); memory = telemetryPercent(memory); storage = telemetryPercent(storage);
  return { timestamp: Date.now(), cpuPercent: cpu, memoryPercent: memory, storagePercent: storage, supported: cpu !== null || memory !== null || storage !== null, source, error: cpu === null && memory === null && storage === null ? 'This device did not expose supported CPU, memory or storage data.' : undefined };
}
function cleanInteractiveCapture(output: string, command: string) { const lines = output.replace(/\x1b\][^\x07]*(?:\x07|\x1b\\)/g, '').replace(/\x1b\[[0-?]*[ -\/]*[@-~]/g, '').replace(/.{0,12}(?:--More--|---\(more[^)]*\)---).{0,12}/gi, '').replace(/[\b\x00]/g, '').replace(/\r/g, '').split('\n'); const commandIndex = lines.findIndex(line => line.trim().endsWith(command)); if (commandIndex >= 0) lines.splice(0, commandIndex + 1); while (lines.length && (!lines.at(-1)?.trim() || /^[\w.@()/:~-]+[>#$]\s*$/.test(lines.at(-1)!.trim()))) lines.pop(); return lines.join('\n').trim(); }
...
ipcMain.handle('data:load', () => ({ folders: stored.folders, sessions: stored.sessions, macros: stored.macros, macroFolders: stored.macroFolders, serialProfiles: stored.serialProfiles, inventorySettings: stored.inventorySettings, uiSettings: stored.uiSettings, credentialProfileMappings: stored.credentialProfileMappings }));
ipcMain.handle('update:status', () => publishUpdateStatus({}));
ipcMain.handle('update:settings', (_event, input?: UpdateSettings) => { if (input) { if (typeof input.automaticChecks !== 'boolean' || !['main', 'experimental'].includes(input.branch)) throw new Error('Invalid update preference.'); stored.updateSettings = { automaticChecks: input.automaticChecks, branch: input.branch }; availableDowngrades.clear(); if (app.isPackaged && !portableBuild) restoreDefaultUpdateFeed(); writeStore(); } return stored.updateSettings; });
ipcMain.handle('update:check', async () => { if (!app.isPackaged || portableBuild) return publishUpdateStatus({ status: 'unsupported', message: portableBuild ? 'Download the latest portable build from GitHub Releases.' : 'Update checks are only available in packaged builds.' }); restoreDefaultUpdateFeed(); publishUpdateStatus({ downgrade: false }); await autoUpdater.checkForUpdates(); return updateStatus; });
ipcMain.handle('update:list-releases', () => listDowngradeReleases());
ipcMain.handle('update:select-release', async (_event, tag: string) => { if (!app.isPackaged || portableBuild) return publishUpdateStatus({ status: 'unsupported', message: 'Downgrades require an installed build of HedgeCon.' }); if (typeof tag !== 'string' || tag.length > 100) throw new Error('Invalid release selection.'); if (!availableDowngrades.has(tag)) await listDowngradeReleases(); const release = availableDowngrades.get(tag); if (!release) throw new Error('That release is unavailable for the selected branch or platform. Refresh the release list and try again.'); selectedUpdateRelease = release; autoUpdater.allowPrerelease = true; autoUpdater.allowDowngrade = true; autoUpdater.setFeedURL({ provider: 'generic', url: `https://github.com/Releah/HedgeCon/releases/download/${encodeURIComponent(release.tag)}` }); publishUpdateStatus({ status: 'checking', availableVersion: release.version, releaseNotes: undefined, progress: undefined, message: `Checking version ${release.version}…`, downgrade: true }); await autoUpdater.checkForUpdates(); return updateStatus; });
...
ipcMain.handle('help:regex101', () => shell.openExternal('https://regex101.com/'));
ipcMain.handle('feedback:open-issue', async (_event, input: any) => { if (!input || !['bug', 'feature'].includes(input.type) || typeof input.title !== 'string' || !input.title.trim() || input.title.length > 180 || typeof input.description !== 'string' || !input.description.trim() || input.description.length > 10_000) throw new Error('Enter a concise title and description.'); const clean = (value: unknown, maximum = 10_000) => typeof value === 'string' ? value.trim().slice(0, maximum) : ''; const installation = portableBuild ? 'Portable executable' : app.isPackaged ? 'Installed build' : 'Development build'; const environment = `HedgeCon ${app.getVersion()}\n${process.platform} ${process.getSystemVersion()} (${process.arch})\n${installation}`; const body = input.type === 'bug' ? `## Problem description\n${clean(input.description)}\n\n## Steps to reproduce\n${clean(input.steps) || '_Not provided_'}\n\n## Expected behaviour\n${clean(input.expected) || '_Not provided_'}\n\n## Actual behaviour\n${clean(input.actual) || '_Not provided_'}\n\n## Environment\n${environment}\n\n## Additional context\n${clean(input.context) || '_None_'}\n\n---\nSubmitted from HedgeCon. The reporter confirmed that secrets and identifying infrastructure details were removed.` : `## Problem or limitation\n${clean(input.description)}\n\n## Proposed solution\n${clean(input.expected) || '_Not provided_'}\n\n## Alternatives considered\n${clean(input.alternatives) || '_None_'}\n\n## Additional context\n${clean(input.context) || '_None_'}\n\n## Environment\n${environment}\n\n---\nSubmitted from HedgeCon.`; if (body.length > 30_000) throw new Error('The report is too long. Shorten it and try again.'); const prefix = input.type === 'bug' ? '[Bug]' : '[Feature]'; const labels = input.type === 'bug' ? 'bug,needs-triage' : 'enhancement,needs-triage'; const url = new URL('https://github.com/Releah/HedgeCon/issues/new'); url.searchParams.set('title', `${prefix}: ${input.title.trim()}`); url.searchParams.set('labels', labels); url.searchParams.set('body', body); await shell.openExternal(url.toString()); return true; });
...
ipcMain.handle('data:save', (_event, data: { folders: unknown[]; sessions: unknown[]; macros?: unknown[]; macroFolders?: unknown[]; serialProfiles?: unknown[]; uiSettings?: unknown; credentialProfileMappings?: Record<string, string> }) => {
  ...
  if (data.serialProfiles && (!Array.isArray(data.serialProfiles) || data.serialProfiles.length > 100 || data.serialProfiles.some((profile: any) => !profile || typeof profile.id !== 'string' || typeof profile.name !== 'string' || !profile.name.trim() || profile.name.length > 80 || !Number.isInteger(profile.baudRate) || profile.baudRate < 50 || profile.baudRate > 4_000_000 || ![5, 6, 7, 8].includes(profile.dataBits) || ![1, 1.5, 2].includes(profile.stopBits) || !['none', 'even', 'odd', 'mark', 'space'].includes(profile.parity)))) throw new Error('Invalid serial profiles.');
  if (data.credentialProfileMappings && (typeof data.credentialProfileMappings !== 'object' || Array.isArray(data.credentialProfileMappings) || Object.keys(data.credentialProfileMappings).length > 500 || Object.entries(data.credentialProfileMappings).some(([profile, credentialId]) => !/^[A-Za-z0-9._ -]{1,120}$/.test(profile) || typeof credentialId !== 'string' || credentialId.length > 200))) throw new Error('Invalid credential profile mappings.'); stored.folders = data.folders; stored.sessions = data.sessions; stored.macros = data.macros ?? []; stored.macroFolders = data.macroFolders ?? []; stored.serialProfiles = data.serialProfiles ?? []; stored.uiSettings = data.uiSettings; stored.credentialProfileMappings = data.credentialProfileMappings ?? {}; writeStore(); return data;
});
...
ipcMain.handle('wiki:list', () => { const repository = getRepository(); if (!stored.wikiGuideSeeded) { const relativePath = 'wiki/general/markdown-guide.md'; const target = path.join(repository.localPath, ...relativePath.split('/')); if (!fs.existsSync(target)) { fs.mkdirSync(path.dirname(target), { recursive: true }); fs.writeFileSync(target, starterWiki[relativePath], 'utf8'); } stored.wikiGuideSeeded = true; writeStore(); } return [...listMarkdownPages(repository), ...listPrivateWikiPages()]; });
ipcMain.handle('wiki:list-folders', () => [...listWikiFolders(getRepository()), ...listPrivateWikiFolders()]);
ipcMain.handle('wiki:read', (_event, relativePath: string) => { const target = wikiStoragePath(relativePath); const stats = fs.statSync(target); if (!stats.isFile() || stats.size > 2 * 1024 * 1024 || !target.toLowerCase().endsWith('.md')) throw new Error('Wiki pages must be Markdown files no larger than 2 MB.'); return fs.readFileSync(target, 'utf8'); });
ipcMain.handle('wiki:write', (_event, relativePath: string, contents: string) => { if (typeof contents !== 'string' || Buffer.byteLength(contents, 'utf8') > 2 * 1024 * 1024 || /-----BEGIN [A-Z ]*PRIVATE KEY-----/.test(contents)) throw new Error('The page is too large or appears to contain a private key.'); const target = wikiStoragePath(relativePath); if (!target.toLowerCase().endsWith('.md')) throw new Error('Wiki pages must use the .md extension.'); fs.mkdirSync(path.dirname(target), { recursive: true, mode: 0o700 }); fs.writeFileSync(target, contents, { encoding: 'utf8', mode: 0o600 }); return true; });
ipcMain.handle('wiki:write-image', (_event, pagePath: string, originalName: string, bytes: Uint8Array) => {
  if (typeof pagePath !== 'string' || !pagePath.toLowerCase().endsWith('.md') || typeof originalName !== 'string' || !(bytes instanceof Uint8Array) || bytes.byteLength < 8 || bytes.byteLength > 10 * 1024 * 1024) throw new Error('Invalid Wiki image. Images must be no larger than 10 MB.');
  const data = Buffer.from(bytes);
  const formats = data.subarray(0, 8).equals(Buffer.from([137,80,78,71,13,10,26,10])) ? { extension: 'png', mime: 'image/png' } : data[0] === 0xff && data[1] === 0xd8 && data[2] === 0xff ? { extension: 'jpg', mime: 'image/jpeg' } : data.subarray(0, 6).toString('ascii').match(/^GIF8[79]a$/) ? { extension: 'gif', mime: 'image/gif' } : data.subarray(0, 4).toString('ascii') === 'RIFF' && data.subarray(8, 12).toString('ascii') === 'WEBP' ? { extension: 'webp', mime: 'image/webp' } : null;
  if (!formats) throw new Error('Paste a PNG, JPEG, GIF, or WebP image.');
  const stem = path.basename(originalName, path.extname(originalName)).replace(/[^a-z0-9_-]+/gi, '-').replace(/^-+|-+$/g, '').slice(0, 60) || 'pasted-image';
  const pageDirectory = path.posix.dirname(pagePath); const assetDirectory = `${pageDirectory}/assets`; let filename = `${stem}.${formats.extension}`; let relativePath = `${assetDirectory}/${filename}`; let suffix = 2;
  while (fs.existsSync(wikiStoragePath(relativePath))) { filename = `${stem}-${suffix++}.${formats.extension}`; relativePath = `${assetDirectory}/${filename}`; }
  const target = wikiStoragePath(relativePath); fs.mkdirSync(path.dirname(target), { recursive: true, mode: 0o700 }); fs.writeFileSync(target, data, { mode: 0o600 });
  return { path: relativePath, markdownPath: `assets/${filename}` };
});
ipcMain.handle('wiki:read-image', (_event, pagePath: string, imagePath: string) => {
  if (typeof pagePath !== 'string' || !pagePath.toLowerCase().endsWith('.md') || typeof imagePath !== 'string' || imagePath.length > 4096 || /^(?:[a-z]+:|\/|\\)/i.test(imagePath)) throw new Error('Invalid Wiki image path.');
  const relativePath = path.posix.normalize(path.posix.join(path.posix.dirname(pagePath), imagePath)); const target = wikiStoragePath(relativePath); const stats = fs.statSync(target); if (!stats.isFile() || stats.size > 10 * 1024 * 1024) throw new Error('Wiki image is unavailable or too large.');
  const extension = path.extname(target).toLowerCase(); const mime = extension === '.png' ? 'image/png' : ['.jpg', '.jpeg'].includes(extension) ? 'image/jpeg' : extension === '.gif' ? 'image/gif' : extension === '.webp' ? 'image/webp' : null; if (!mime) throw new Error('Unsupported Wiki image format.');
  return `data:${mime};base64,${fs.readFileSync(target).toString('base64')}`;
});
ipcMain.handle('wiki:create', (_event, rawSection: string, name: string, parentPath?: string) => { if (!['general', 'vendors', 'private'].includes(rawSection)) throw new Error('Choose a valid Wiki section.'); const slug = wikiSlug(name); if (rawSection === 'private' || parentPath === privateWikiPrefix || parentPath?.startsWith(`${privateWikiPrefix}/`)) { const parent = parentPath || privateWikiPrefix; const parentTarget = safePrivateNotesPath(`${parent}/.keep`); fs.mkdirSync(path.dirname(parentTarget), { recursive: true, mode: 0o700 }); const relativePath = `${parent}/${slug}.md`; const target = safePrivateNotesPath(relativePath); if (fs.existsSync(target)) throw new Error('A page with that name already exists.'); fs.writeFileSync(target, `# ${name.trim()}\n\n`, { encoding: 'utf8', flag: 'wx', mode: 0o600 }); return { path: relativePath, title: name.trim(), section: 'private' }; } const section = rawSection as 'general' | 'vendors'; const folder = validateWikiFolder(section, parentPath); const relativePath = `${folder.relative}/${slug}.md`; const target = safeRepoPath(relativePath); if (fs.existsSync(target)) throw new Error('A page with that name already exists.'); fs.writeFileSync(target, `# ${name.trim()}\n\n`, { encoding: 'utf8', flag: 'wx' }); return { path: relativePath, title: name.trim(), section }; });
ipcMain.handle('wiki:create-folder', (_event, rawSection: string, name: string, parentPath?: string) => { if (!['general', 'vendors', 'private'].includes(rawSection)) throw new Error('Choose a valid Wiki section.'); const slug = wikiSlug(name); if (rawSection === 'private' || parentPath === privateWikiPrefix || parentPath?.startsWith(`${privateWikiPrefix}/`)) { const parent = parentPath || privateWikiPrefix; const relativePath = `${parent}/${slug}`; const target = safePrivateNotesPath(relativePath); if (fs.existsSync(target)) throw new Error('A folder with that name already exists.'); fs.mkdirSync(target, { recursive: true, mode: 0o700 }); return { path: relativePath, name: name.trim(), section: 'private' }; } const section = rawSection as 'general' | 'vendors'; const parent = validateWikiFolder(section, parentPath); const relativePath = `${parent.relative}/${slug}`; const target = safeRepoPath(relativePath); if (fs.existsSync(target)) throw new Error('A folder with that name already exists.'); fs.mkdirSync(target); fs.writeFileSync(path.join(target, '.gitkeep'), '', { flag: 'wx' }); return { path: relativePath, name: name.trim(), section }; });
ipcMain.handle('wiki:rename-folder', (_event, folderPath: string, name: string) => { if (folderPath === privateWikiPrefix || folderPath?.startsWith(`${privateWikiPrefix}/`)) { if (folderPath === privateWikiPrefix) throw new Error('The Private Notes root cannot be renamed.'); const source = safePrivateNotesPath(folderPath); if (!fs.existsSync(source) || !fs.statSync(source).isDirectory()) throw new Error('The private folder no longer exists.'); const relativePath = `${path.posix.dirname(folderPath)}/${wikiSlug(name)}`; const target = safePrivateNotesPath(relativePath); if (target !== source && fs.existsSync(target)) throw new Error('A folder with that name already exists.'); fs.renameSync(source, target); return { path: relativePath, name: name.trim(), section: 'private' }; } const section: 'general' | 'vendors' | null = folderPath?.startsWith('wiki/general/') ? 'general' : folderPath?.startsWith('vendors/') ? 'vendors' : null; if (!section) throw new Error('Only Wiki folders can be renamed.'); const folder = validateWikiFolder(section, folderPath); if (folder.relative === wikiRoot(section)) throw new Error('The main Wiki sections cannot be renamed.'); const relativePath = `${path.posix.dirname(folder.relative)}/${wikiSlug(name)}`; const target = safeRepoPath(relativePath); if (target !== folder.absolute && fs.existsSync(target)) throw new Error('A folder with that name already exists.'); fs.renameSync(folder.absolute, target); return { path: relativePath, name: name.trim(), section }; });
ipcMain.handle('wiki:delete-folder', (_event, folderPath: string) => { if (folderPath?.startsWith(`${privateWikiPrefix}/`)) { const folder = safePrivateNotesPath(folderPath); if (!fs.existsSync(folder) || !fs.statSync(folder).isDirectory()) throw new Error('The private folder no longer exists.'); const parent = path.dirname(folder); const entries = fs.readdirSync(folder, { withFileTypes: true }); for (const entry of entries) if (fs.existsSync(path.join(parent, entry.name))) throw new Error(`Cannot delete this folder because “${entry.name}” already exists one level up.`); const movedPages: Array<{ from: string; to: string }> = []; for (const entry of entries) { const from = path.join(folder, entry.name); const to = path.join(parent, entry.name); if (entry.isFile() && entry.name.toLowerCase().endsWith('.md')) movedPages.push({ from: `${folderPath}/${entry.name}`, to: `${path.posix.dirname(folderPath)}/${entry.name}` }); fs.renameSync(from, to); } fs.rmdirSync(folder); return { parentPath: path.posix.dirname(folderPath), movedPages }; } const section: 'general' | 'vendors' | null = folderPath?.startsWith('wiki/general/') ? 'general' : folderPath?.startsWith('vendors/') ? 'vendors' : null; if (!section) throw new Error('Only Wiki folders can be deleted.'); const folder = validateWikiFolder(section, folderPath); if (folder.relative === wikiRoot(section)) throw new Error('The main Wiki sections cannot be deleted.'); const parent = path.dirname(folder.absolute); const entries = fs.readdirSync(folder.absolute, { withFileTypes: true }).filter(entry => entry.name !== '.gitkeep'); for (const entry of entries) if (fs.existsSync(path.join(parent, entry.name))) throw new Error(`Cannot delete this folder because “${entry.name}” already exists one level up.`); const movedPages: Array<{ from: string; to: string }> = []; const repository = getRepository(); for (const entry of entries) { const from = path.join(folder.absolute, entry.name); const to = path.join(parent, entry.name); if (entry.isFile() && entry.name.toLowerCase().endsWith('.md')) movedPages.push({ from: path.relative(repository.localPath, from).split(path.sep).join('/'), to: path.relative(repository.localPath, to).split(path.sep).join('/') }); fs.renameSync(from, to); } const keep = path.join(folder.absolute, '.gitkeep'); if (fs.existsSync(keep)) fs.unlinkSync(keep); fs.rmdirSync(folder.absolute); return { parentPath: path.posix.dirname(folder.relative), movedPages }; });
ipcMain.handle('wiki:move-page', (_event, sourcePath: string, targetFolder: string) => { if (typeof sourcePath !== 'string' || typeof targetFolder !== 'string' || !sourcePath.toLowerCase().endsWith('.md')) throw new Error('Invalid Wiki page move.'); if (sourcePath.startsWith(`${privateWikiPrefix}/`)) { if (targetFolder !== privateWikiPrefix && !targetFolder.startsWith(`${privateWikiPrefix}/`)) throw new Error('Private pages can only move within Private Notes.'); const source = safePrivateNotesPath(sourcePath); const relativePath = `${targetFolder}/${path.posix.basename(sourcePath)}`; const target = safePrivateNotesPath(relativePath); if (!fs.existsSync(source) || !fs.statSync(source).isFile()) throw new Error('The private page no longer exists.'); if (relativePath !== sourcePath && fs.existsSync(target)) throw new Error('A page with that filename already exists in the target folder.'); if (relativePath !== sourcePath) fs.renameSync(source, target); const contents = fs.readFileSync(target, 'utf8'); return { path: relativePath, title: wikiPageTitle(contents, path.basename(target)), section: 'private' }; } const section: 'general' | 'vendors' | null = sourcePath.startsWith('wiki/general/') ? 'general' : sourcePath.startsWith('vendors/') ? 'vendors' : null; if (!section) throw new Error('Session notes cannot be moved.'); const folder = validateWikiFolder(section, targetFolder); const source = safeRepoPath(sourcePath); if (!fs.existsSync(source) || !fs.statSync(source).isFile()) throw new Error('The Wiki page no longer exists.'); const relativePath = `${folder.relative}/${path.posix.basename(sourcePath)}`; if (relativePath === sourcePath) { const contents = fs.readFileSync(source, 'utf8'); return { path: sourcePath, title: wikiPageTitle(contents, path.basename(source)), section }; } const target = safeRepoPath(relativePath); if (fs.existsSync(target)) throw new Error('A page with that filename already exists in the target folder.'); fs.renameSync(source, target); const contents = fs.readFileSync(target, 'utf8'); return { path: relativePath, title: wikiPageTitle(contents, path.basename(target)), section }; });
ipcMain.handle('wiki:session-page', (_event, sessionId: string, sessionName: string, host: string, identity?: any) => { if (typeof sessionId !== 'string' || !/^[a-z0-9-]{1,100}$/i.test(sessionId) || typeof sessionName !== 'string' || sessionName.length > 200 || typeof host !== 'string' || host.length > 253) throw new Error('Invalid session details.'); const relativePath = `wiki/sessions/${sessionId}/README.md`; const target = safeRepoPath(relativePath); const safeCell = (value: unknown) => typeof value === 'string' && value.trim() ? value.trim().replace(/\|/g, '\\|').slice(0, 300) : 'TBC'; const information = `<!-- hedgecon-device-info:start -->\n## Device information\n\n| Field | Value |\n| --- | --- |\n| Hostname | ${safeCell(identity?.hostname)} |\n| IP address / host | ${safeCell(host)} |\n| Make | ${safeCell(identity?.vendor)} |\n| Model / OS | ${safeCell(identity?.product)} |\n| Version | ${safeCell(identity?.version)} |\n<!-- hedgecon-device-info:end -->`; if (!fs.existsSync(target)) { fs.mkdirSync(path.dirname(target), { recursive: true }); fs.writeFileSync(target, `# ${sessionName}\n\n${information}\n\n## Notes\n\n`, { encoding: 'utf8', flag: 'wx' }); } else { const source = fs.readFileSync(target, 'utf8'); const managed = /<!-- hedgecon-device-info:start -->[\s\S]*?<!-- hedgecon-device-info:end -->/; const updated = managed.test(source) ? source.replace(managed, information) : source.replace(/^(#[^\n]*\n)/, `$1\n${information}\n`); if (updated !== source) fs.writeFileSync(target, updated, 'utf8'); } return { path: relativePath, title: sessionName, section: 'sessions' }; });
ipcMain.handle('wiki:private-session-page', (_event, sessionId: string, sessionName: string, host: string) => { if (typeof sessionId !== 'string' || !/^[a-z0-9-]{1,100}$/i.test(sessionId) || typeof sessionName !== 'string' || sessionName.length > 200 || typeof host !== 'string' || host.length > 253) throw new Error('Invalid session details.'); const relativePath = `private/sessions/${sessionId}.md`; const target = safePrivateNotesPath(relativePath); if (!fs.existsSync(target)) { fs.mkdirSync(path.dirname(target), { recursive: true, mode: 0o700 }); fs.writeFileSync(target, `# ${sessionName} — Private notes\n\n**Host:** \`${host}\`\n\n`, { encoding: 'utf8', flag: 'wx', mode: 0o600 }); } return { path: relativePath, title: sessionName, section: 'private' }; });
...
ipcMain.handle('credentials:copy-field', (_event, id: string, field: 'username' | 'password') => {
  if (typeof id !== 'string' || !['username', 'password'].includes(field)) throw new Error('Invalid credential copy request.');
  const credential = stored.credentialSets.find(item => item.id === id);
  if (!credential) throw new Error('Credential set not found.');
  if (field === 'username') clipboard.writeText(credential.username);
  else {
    if (credential.authMethod !== 'password') throw new Error('This credential set does not contain a password.');
    if (!credential.encryptedSecret) throw new Error('No password is saved in this credential set.');
    requireSecureStorage('The saved credential password');
    clipboard.writeText(safeStorage.decryptString(Buffer.from(credential.encryptedSecret, 'base64')));
  }
  return true;
});
...
ipcMain.handle('ssh:connect', async (_event, request: any) => {
  ...
  const id = request.connectionId || crypto.randomUUID(); const client = new Client(); connections.set(id, { client, name: typeof request.name === 'string' ? request.name.slice(0, 200) : request.host, host: request.host, platform: typeof request.platform === 'string' ? request.platform : 'unspecified' });
  ...
  }).on('error', (err) => { stopConnectionTunnels(id); closeSessionLog(id); const authenticationFailure = (err as any).level === 'client-authentication' || /authentication methods failed|authentication failure/i.test(err.message); const algorithmFailure = !legacyEnabled && /(?:handshake failed:\s*)?no matching (?:key exchange|host key|cipher|hmac|mac)|unsupported (?:key exchange|host key|cipher|hmac|mac)|no supported (?:key exchange|host key|cipher|hmac|mac)/i.test(err.message); send(id, authenticationFailure ? 'auth-error' : algorithmFailure ? 'legacy-required' : 'error', algorithmFailure ? `Modern SSH negotiation failed for ${hostKey}. This host may require obsolete key-exchange, host-key, cipher or MAC algorithms.` : err.message); connections.delete(id); })
...
ipcMain.handle('ssh:telemetry', async (_event, host: string, suppliedIdentity?: { vendor?: unknown; product?: unknown; platform?: unknown }) => {
  if (typeof host !== 'string' || !host.trim()) throw new Error('Invalid telemetry host.');
  const connection = [...connections.values()].find(item => item.host === host); if (!connection) throw new Error('Open the SSH session before collecting device health.');
  const saved = (stored.sessions as Array<any>).find(item => item?.host === host)?.detectedIdentity;
  const identity = suppliedIdentity && typeof suppliedIdentity === 'object' ? suppliedIdentity : saved;
  const vendor = typeof identity?.vendor === 'string' ? identity.vendor : ''; const product = typeof identity?.product === 'string' ? identity.product : '';
  let output = ''; let source = 'SSH telemetry';
  if (/juniper|junos/i.test(`${vendor} ${product}`)) { source = 'Junos · 10s'; output = await sshShellProbe(connection.client, ['show chassis routing-engine | no-more', 'show system storage | no-more'], 8000).catch(() => ''); }
  else if (/cisco|ios|nx-os|asa/i.test(`${vendor} ${product}`)) { source = 'Cisco · 10s'; output = await sshShellProbe(connection.client, ['terminal length 0', 'show processes cpu', 'show processes memory', 'dir flash:'], 8000).catch(() => ''); }
  else if (/dell|os10|smartfabric/i.test(`${vendor} ${product}`)) { source = 'OS10 · 10s'; output = await sshShellProbe(connection.client, ['terminal length 0', 'show processes node-id 1', 'show system', 'show file-systems'], 8000).catch(() => ''); }
  else { source = 'Linux · 10s'; output = await sshExec(connection.client, `printf 'HEDGE_CPU|'; top -bn1 2>/dev/null | awk '/Cpu\\(s\\)|%Cpu/{for(i=1;i<=NF;i++)if($i~/id,?$/){gsub(/,/,"",$(i-1));print 100-$(i-1);exit}}'; printf 'HEDGE_MEM|'; free 2>/dev/null | awk '/^Mem:/{print ($2-$7)*100/$2}'; printf 'HEDGE_DISK|'; df -P / 2>/dev/null | awk 'NR==2{gsub(/%/,"",$5);print $5}'`, 7000).catch(() => ''); }
  return parseTelemetry(output, source);
});
...
ipcMain.handle('compare:open-text', async () => {
  const chosen = await dialog.showOpenDialog(mainWindow!, { title: 'Open text or configuration file', properties: ['openFile'], filters: [{ name: 'Text and configuration files', extensions: ['txt', 'cfg', 'conf', 'config', 'log', 'ini', 'yaml', 'yml', 'json', 'xml'] }, { name: 'All files', extensions: ['*'] }] });
  if (chosen.canceled) return null;
  const filePath = chosen.filePaths[0]; const stats = fs.statSync(filePath);
  if (!stats.isFile() || stats.size > 5 * 1024 * 1024) throw new Error('Compare files must be no larger than 5 MB.');
  const bytes = fs.readFileSync(filePath); if (bytes.includes(0)) throw new Error('This appears to be a binary file. Choose a text configuration file.');
  return { name: path.basename(filePath), contents: bytes.toString('utf8') };
});
ipcMain.handle('compare:connections', () => [...connections.entries()].filter(([, entry]) => Boolean(entry.stream)).map(([id, entry]) => ({ id, name: entry.name, host: entry.host, platform: entry.platform })));
ipcMain.handle('compare:capture', async (_event, connectionId: string, rawCommand: string) => {
  ...
  let output = ''; let execFailure: unknown;
  try { output = await sshExec(connection.client, command, 12000, 2 * 1024 * 1024); } catch (error) { execFailure = error; }
  if (!output.trim() && execFailure && /channel open|open failed|administratively prohibited/i.test(String(execFailure))) output = await sshInteractiveCapture(connectionId, connection.stream, command);
  else if (!output.trim()) { try { output = await sshShellProbe(connection.client, [command], 15000, 2 * 1024 * 1024); } catch { output = await sshInteractiveCapture(connectionId, connection.stream, command); } }
...
ipcMain.handle('serial:connect', async (_event, request: any) => {
  ...
  const id = request.connectionId; if (serialConnections.has(id)) throw new Error('This serial connection is already open.');
  const port = new SerialPort({ path: request.path, baudRate: request.baudRate, dataBits: request.dataBits, stopBits: request.stopBits, parity: request.parity, autoOpen: false }); serialConnections.set(id, port);
  const emit = (type: string, data: string) => sendToRenderer('serial:event', { connectionId: id, type, data });
  port.on('data', (data: Buffer) => emit('data', data.toString())); port.on('error', error => emit('error', error.message)); port.on('close', () => { serialConnections.delete(id); emit('closed', 'Serial connection closed'); });
  await new Promise<void>((resolve, reject) => port.open(error => error ? reject(error) : resolve())); emit('status', `Connected to ${request.path} at ${request.baudRate} baud`); return { connectionId: id };
});
ipcMain.on('serial:write', (_event, id: string, data: string) => { if (typeof data === 'string' && data.length <= 1024 * 1024) serialConnections.get(id)?.write(data); });
ipcMain.on('serial:disconnect', (_event, id: string) => { const port = serialConnections.get(id); if (port?.isOpen) port.close(); else serialConnections.delete(id); });
ipcMain.handle('ping:start', (_event, host: string, requestedId: string) => {
  ...
  const probe = () => {
    ...
    monitor.child = execFile('ping', args, { timeout: 1800, windowsHide: true, maxBuffer: 64 * 1024 }, (error, stdout) => {
      ...
      if (monitor.stopped) return;
      const match = stdout.match(/time[=<]?\s*([\d.]+)\s*ms/i); const reachable = !error && Boolean(match); const latencyMs = reachable ? Number(match![1]) : null;
      sendToRenderer('ping:sample', { monitorId, timestamp: Date.now(), reachable, latencyMs, error: error?.message || (reachable ? undefined : 'No ICMP echo reply was received from the target.') });
...
ipcMain.handle('socks-monitor:start', (_event, proxyHost: string, proxyPort: number, host: string, port: number, requestedId: string) => {
  if (typeof proxyHost !== 'string' || !isValidHost(proxyHost) || !Number.isInteger(proxyPort) || proxyPort < 1 || proxyPort > 65535 || typeof host !== 'string' || !isValidHost(host) || !Number.isInteger(port) || port < 1 || port > 65535) throw new Error('Invalid SOCKS5 monitor target.');
  const monitorId = typeof requestedId === 'string' && requestedId ? requestedId : crypto.randomUUID(); const monitor = { stopped: false } as { stopped: boolean; timer?: NodeJS.Timeout };
  pingMonitors.set(monitorId, monitor);
  const probe = async () => {
    if (monitor.stopped) return;
    const started = Date.now();
    try { const socket = await connectThroughSocks(proxyHost, proxyPort, host, port, 1500); socket.destroy(); if (!monitor.stopped) sendToRenderer('ping:sample', { monitorId, timestamp: Date.now(), reachable: true, latencyMs: Math.max(1, Date.now() - started) }); }
    catch (error) { if (!monitor.stopped) sendToRenderer('ping:sample', { monitorId, timestamp: Date.now(), reachable: false, latencyMs: null, error: error instanceof Error ? error.message : String(error) }); }
    if (!monitor.stopped) monitor.timer = setTimeout(probe, 1000);
  };
  void probe(); return { monitorId };
});
