...
ipcMain.handle('ssh:identify', async (_event, connectionId: string) => {
  ...
  const windows = await sshExec(connection.client, 'powershell -NoProfile -NonInteractive -Command "$o=Get-CimInstance Win32_OperatingSystem; Write-Output (\'HEDGECON_WINDOWS|\'+$o.Caption+\'|\'+$o.Version)"', 4500).catch(() => '');
  const windowsMatch = windows.match(/HEDGECON_WINDOWS\|([^\r\n|]+)\|([^\r\n]+)/); if (windowsMatch) return { platform: 'windows', vendor: 'Microsoft', product: windowsMatch[1].trim(), version: windowsMatch[2].trim(), confidence: 'high', evidence: windowsMatch[0].slice(0, 500) };
  const unix = await sshExec(connection.client, `sh -c 'cat /etc/os-release 2>/dev/null; printf "HEDGECON_UNAME="; uname -srm 2>/dev/null'`, 4500).catch(() => '');
  if (/HEDGECON_UNAME=Linux/i.test(unix) || /(?:^|\n)(?:ID|NAME)=/m.test(unix)) { const name = unix.match(/(?:^|\n)PRETTY_NAME=[\"']?([^\r\n\"']+)/)?.[1] || unix.match(/(?:^|\n)NAME=[\"']?([^\r\n\"']+)/)?.[1] || 'Linux'; const version = unix.match(/(?:^|\n)VERSION_ID=[\"']?([^\r\n\"']+)/)?.[1] || unix.match(/HEDGECON_UNAME=Linux\s+([^\r\n]+)/)?.[1] || ''; return { platform: 'linux', vendor: name.split(/\s+/)[0], product: name.trim(), version: version.trim(), confidence: 'high', evidence: unix.slice(0, 1000) }; }
  const network = await sshExec(connection.client, 'show version', 6000).catch(() => ''); const text = network.replace(/\x1b\[[0-?]*[ -\/]*[@-~]/g, ' ');
  const signatures: Array<[RegExp, string, string]> = [[/Cisco IOS|NX-OS|Cisco Adaptive Security/i, 'Cisco', 'Cisco network device'], [/JUNOS|Junos:/i, 'Juniper', 'Juniper network device'], [/Arista|EOS version/i, 'Arista', 'Arista network device'], [/FortiGate|FortiOS/i, 'Fortinet', 'FortiGate'], [/RouterOS/i, 'MikroTik', 'RouterOS'], [/ProCurve|ArubaOS|Aruba /i, 'HPE Aruba', 'Aruba network device']]; const found = signatures.find(([pattern]) => pattern.test(text)); if (found) return { platform: 'network', vendor: found[1], product: found[2], version: text.match(/(?:Version|version|Junos:)\s*[: ]?([\w().-]+)/)?.[1] || '', confidence: 'high', evidence: text.slice(0, 1000) };
  return { platform: 'unspecified', vendor: '', product: 'Unknown SSH device', version: '', confidence: 'low', evidence: (windows + unix + text).trim().slice(0, 1000) };
});
