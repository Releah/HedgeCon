...
function listPrivateWikiPages() { const pages: Array<{ path: string; title: string; section: 'general' }> = []; const root = path.join(privateNotesRoot(), 'general'); if (!fs.existsSync(root)) return pages; const pending = [root]; while (pending.length) { const directory = pending.pop()!; for (const entry of fs.readdirSync(directory, { withFileTypes: true }).slice(0, 2000)) { if (entry.isSymbolicLink()) continue; const full = path.join(directory, entry.name); if (entry.isDirectory()) pending.push(full); else if (entry.isFile() && entry.name.toLowerCase().endsWith('.md') && fs.statSync(full).size <= 2 * 1024 * 1024) { const suffix = path.relative(root, full).split(path.sep).join('/'); pages.push({ path: `${privateWikiPrefix}/${suffix}`, title: wikiPageTitle(fs.readFileSync(full, 'utf8'), entry.name), section: 'general' }); } } } return pages.sort((a, b) => a.title.localeCompare(b.title)); }
function listPrivateWikiFolders() { const folders: Array<{ path: string; name: string; section: 'general' }> = [{ path: privateWikiPrefix, name: 'Private Notes · Local only', section: 'general' }]; const root = path.join(privateNotesRoot(), 'general'); if (!fs.existsSync(root)) return folders; const pending = [root]; while (pending.length) { const directory = pending.pop()!; for (const entry of fs.readdirSync(directory, { withFileTypes: true }).slice(0, 2000)) { if (!entry.isDirectory() || entry.isSymbolicLink()) continue; const full = path.join(directory, entry.name); folders.push({ path: `${privateWikiPrefix}/${path.relative(root, full).split(path.sep).join('/')}`, name: entry.name.replace(/[-_]/g, ' '), section: 'general' }); pending.push(full); } } return folders.sort((a, b) => a.path.localeCompare(b.path)); }
