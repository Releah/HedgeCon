...
export default function MacroBoard({ session, macros, macroFolders, folders, pinned = false, onPinnedChange, onPaste, onClose, onManage }: { session: Session; macros: CommandMacro[]; macroFolders: MacroFolder[]; folders: Folder[]; pinned?: boolean; onPinnedChange?: (pinned: boolean) => void; onPaste: (command: string, runImmediately: boolean) => void; onClose: () => void; onManage: () => void }) {
  ...
  return <aside className="macro-panel"><div className="macro-panel-divider" onPointerDown={beginResize} /><header><div><small>MACROS</small><strong>{session.name}</strong></div><div><button className={`macro-pin${pinned ? ' active' : ''}`} onClick={() => onPinnedChange?.(!pinned)} title={pinned ? 'Unpin macros from all sessions' : 'Keep macros open across sessions'} aria-label={pinned ? 'Unpin macros from all sessions' : 'Keep macros open across sessions'} aria-pressed={pinned}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 3h8l-1.5 6 3 3v2H13v7l-2-2v-5H6.5v-2l3-3L8 3z" /></svg></button><button onClick={onManage} title="Manage macros" aria-label="Manage macros">⚙</button><button onClick={onClose} title="Close macros" aria-label="Close macros">×</button></div></header><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search macros..." /><div className="macro-panel-label">{query ? 'RESULTS' : 'FAVOURITES'}</div><div className="macro-session-tree">{unfiled.map(macroButton)}{renderFolders()}{!visible.length && <p>{query ? 'No matching macros.' : 'No favourites for this session. Search all macros or manage the library.'}</p>}</div>{running && <MacroRunDialog macro={running} builtIns={builtIns} onCancel={() => setRunning(null)} onRun={command => { onPaste(command, Boolean(running.runImmediately)); setRunning(null); }} />}</aside>;
}
