...
export default function PingMonitor({ host, onClose }: { host: string; onClose: () => void }) {
  const monitorId = useRef(crypto.randomUUID()); const [samples, setSamples] = useState<PingSample[]>([]);
  ...
  useEffect(() => {
    ...
    void window.hedge.startPing(host, monitorId.current);
  ...
  return <section className="ping-monitor"><header><div className={`ping-state ${metrics.reachable === false ? 'offline' : metrics.reachable === true ? 'online' : ''}`}><span />{metrics.reachable === null ? 'Starting ping…' : metrics.reachable ? 'Online' : `Unresponsive for ${duration(Date.now() - (metrics.downSince ?? Date.now()))}`}</div><div className="ping-stats"><span>Host <strong>{host}</strong></span><span>Now <strong>{latest?.latencyMs === null || latest?.latencyMs === undefined ? '—' : `${latest.latencyMs.toFixed(1)} ms`}</strong></span><span>Average <strong>{metrics.average === null ? '—' : `${metrics.average.toFixed(1)} ms`}</strong></span>{metrics.lastOutage !== null && <span>Last outage <strong>{duration(metrics.lastOutage)}</strong></span>}</div><button onClick={onClose} title="Stop ping monitor">×</button></header><div className="ping-chart"><svg viewBox="0 0 600 100" preserveAspectRatio="none" aria-label="Ping response graph"><line x1="0" y1="88" x2="600" y2="88" /><line x1="0" y1="50" x2="600" y2="50" />{samples.map((sample, index) => sample.reachable ? null : <rect key={sample.timestamp} className="ping-outage" x={Math.max(0, index / Math.max(samples.length - 1, 1) * 600 - 3)} y="6" width="7" height="82" />)}<path d={path} /></svg>{samples.some(sample => !sample.reachable) && <div className="outage-label">Red bands indicate no response</div>}</div></section>;
}
