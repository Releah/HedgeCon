...
export default function SerialView({ session, active = true }: { session: Session; active?: boolean }) {
  ...
  useEffect(() => {
    ...
    const remove = window.hedge.onSerialEvent((event: SerialEvent) => { if (event.connectionId !== connectionId) return; if (event.type === 'data') terminal.write(highlightTerminalText(event.data, settingsRef.current.terminalPatterns)); else if (event.type === 'status') terminal.writeln(`\x1b[38;2;103;215;178m${event.data}\x1b[0m`); else terminal.writeln(`\r\n\x1b[38;2;232;126;126m${event.data}\x1b[0m`); });
