...
export default function SerialView({ session, active = true, onActivity = () => {} }: { session: Session; active?: boolean; onActivity?: () => void }) {
  ...
  const [patternRevision, setPatternRevision] = useState(0); const terminalRef = useRef<Terminal | null>(null);
  useEffect(() => { const update = (event: Event) => { settingsRef.current = (event as CustomEvent<UiSettings>).detail; setPatternRevision(value => value + 1); }; window.addEventListener('hedgecon:ui-settings', update); return () => window.removeEventListener('hedgecon:ui-settings', update); }, []);
  useEffect(() => { const terminal = terminalRef.current; if (terminal) refreshTerminalPatternDecorations(terminal, settingsRef.current.terminalPatterns, '\n'.repeat(Math.min(120, terminal.rows))); }, [patternRevision]);
  useEffect(() => {
