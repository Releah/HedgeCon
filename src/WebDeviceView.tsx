...
import type { BrowserEvent, Session } from './types';
...
export default function WebDeviceView({ tabId, session, visible, onClose }: { tabId: string; session: Session; visible: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    void window.hedge.createBrowser(tabId, initialUrl, bounds(element)).catch(reason => setError(reason instanceof Error ? reason.message : String(reason)));
    return () => { remove(); resize.disconnect(); modalObserver.disconnect(); window.hedge.destroyBrowser(tabId); };
